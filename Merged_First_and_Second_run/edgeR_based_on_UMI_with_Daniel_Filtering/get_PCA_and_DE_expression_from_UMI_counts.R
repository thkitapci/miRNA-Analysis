...
setwd("C:/Users/thkit_000/Desktop/miRNA-Analysis/Merged_First_and_Second_run/edgeR_based_on_UMI_with_Daniel_Filtering")
library(FactoMineR)
...
##Filter by a threshold if necessary such that all fields are bigger than the threshold
#threshold=10 #min CPM threshold of 10 in all samples
#myFilter=apply(table_for_PCA, 1, function(x){all(x>threshold)})
#table_for_PCA <- table_for_PCA[myFilter,]

##Filter by a threshold such that 90% of the columns are bigger than zero
#filtered_table=table[rowSums(table > 0) >= 0.9*ncol(table),]  

#Filter such that cpm >1 in at least 4 samples

Threshold=1

check=table_for_PCA>Threshold
keep=which(rowSums(check) >= 4)
...
#Filter based on Daniel's suggestion
...
write.csv(edgeR_result$table,"miRNA_DE_list_based_on_UMI_Daniel_Filtering.csv",quote=FALSE)
